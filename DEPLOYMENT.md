...
- Provisions the Foundry model deployment in Terraform
- Validates the Terraform-managed Foundry model deployment before any Foundry deployment work begins
...
3. Choose options:
   ...
   - `destroy_foundry_agents`: Set to `true` to delete only agents
...
3. Click **Ask AI** — the Web UI calls `/ask` → API → the SQL-backed appointment repository (with Foundry/gateway fallback only if needed). The prompt parser understands follow-up hints like "afternoon" or "after 2 PM", plus weekdays and explicit dates such as "Friday" or "2026-08-17".
...
- `POST /ask` - Natural language scheduling request backed by SQL availability when PostgreSQL is configured
